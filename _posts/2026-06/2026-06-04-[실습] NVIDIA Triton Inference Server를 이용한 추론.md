...
하지만 이 방식은 GPU의 90%를 낭비한다. NVIDIA Triton Inference Server 같은 현대적인 추론 서버는 단순한 모델을 운영 환경 수준의 엔드포인트로 바꾼다. 동적 배칭, 다중 모델 공유, 모델 버전 관리, 내장된 프로메테우스(Prometheus) 메트릭 같은 기능을 제공한다.
...
- **[Continuous batching (Anyscale)](https://www.anyscale.com/blog/continuous-batching-llm-inference)** : 연속 배칭은 LLM에 특화된 동적 배칭의 발전형이고 vLLM, TGI, Triton의 vLLM backend에서 사용된다.
...
224×224 이미지를 분류하는 ResNet 스타일의 합성곱 신경망이다. 동시 클라이언트 부하를 시뮬레이션하고 QPS(초당 질의 수)와 지연시간 백분위수를 측정한다. RTX 3060 Ti 기준 예상 결과는 약 20\~40 QPS이다. 이는 매우 낮은 수치이며 요청 사이에 GPU가 대부분 유휴 상태로 남아 있음을 의미한다. 큐잉 경합이 없기 때문에 p99 지연시간은 평균에 가깝게 나타나지만, 처리량은 심하게 제한된다.
...
## 2-3. 동일한 부하 테스트를 배처 대상으로 다시 실행함
...
지금까지 직접 만든 모델 로딩, 동적 배칭, 요청 큐는 운영 환경에서 `nvcr.io/nvidia/tritonserver`가 수행하는 일과 같다. Triton은 검증된 C++ 바이너리로 구현되어 있으며, gRPC/HTTP, Prometheus 메트릭, 모델 버전 관리, 다중 backend를 제대로 제공한다. 이 모델을 실제 Triton에 배포하려면 다음 단계를 수행한다.
...
```bash
docker run --gpus=all --shm-size=256m -v $PWD/models:/models -p 8000:8000 -p 8001:8001 -p 8002:8002   
...
## 4-1. 동일한 입출력 계약(contract)을 운영 환경용으로 패키징하는 Triton Inference Server 설정
